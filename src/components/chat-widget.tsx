...
type PrefilledQuestion = { id: string; question: string };
...
const ChatView = React.memo(function ChatView({ 
  ...
  prefilledQuestions: PrefilledQuestion[]
}) {
  const showPrefilledQuestions = messages.length <= 1;

  return (
    <>
      <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
        {showPrefilledQuestions && prefilledQuestions.length > 0 && (
          <div className="flex flex-col items-center gap-2 my-4">
            <p className="text-sm text-muted-foreground mb-2">Or try one of these questions:</p>
            <div className="flex flex-wrap justify-center gap-2">
              {prefilledQuestions.map(q => (
                <Button 
                  ...
                  variant="outline" 
                  size="sm"
                  onClick={(e) => handleSendMessage(e, q.question)}
                  disabled={isLoading}
                >
                  {q.question}
                </Button>
...
export default function ChatWidget() {
  ...
  useEffect(() => {
    if (isOpen) {
        const fetchPrefilledQuestions = async () => {
            try {
                const response = await fetch('/api/get-prefilled-questions');
                if (response.ok) {
                    const data = await response.json();
                    setPrefilledQuestions(data);
                }
            } catch (error) {
                console.error('Could not fetch prefilled questions:', error);
            }
        };
        fetchPrefilledQuestions();
    }
  }, [isOpen]);
  ...
  const handleSendMessage = async (e: React.FormEvent, predefinedQuery?: string) => {
    ...
    try {
      ...
      if (!response.ok || !response.body) {
        const errorText = response.ok ? 'Server sent an empty response.' : await response.text();
      ...
      while (!done) {
        ...
        if (!isHeaderParsed) {
          ...
          if (headerEndIndex !== -1) {
            ...
            buffer = buffer.substring(headerEndIndex + 2);
            try {
              ...
              console.error("Could not parse header, treating as content:", e);
              streamedContent += headerPart + '\n\n';
            }
            isHeaderParsed = true;
          }
        }
      ...
      if (fullResponse.trim().toUpperCase() === "IDK") {
